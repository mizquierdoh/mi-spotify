 ...
 */
export default class MusicStoryApi {

	constructor(oauth_consumer_key, oauth_consumer_secret, token, token_secret, version) {
				...
				if (type === 'getConnector' || type === 'searchObject') {
					if (_this.resp.data) {
						...
						data.forEach(function (entry) {
							items.push(new MusicStoryObject(_this, entry, name, keys));
						});
					...
					var res = new MusicStoryObject(_this, _this.resp, name, keys);
					if (typeof callback !== 'undefined')
						callback(res, return_obj);
				}
			});
 ...
 */
function MusicStoryObject(api, item, name, keys) {
	...
	this.getConnector = function (connector, filter, page, count, callback) {
		...
		if (!filter)
			filter = {};
		api._request(object, msobj.id, connector, filter, type, page, count, function () {
			if (connector === 'biographies')
				connector = 'biography';
			else
				connector = connector.substr(0, connector.length - 1);
			api.constructResult(connector, type, callback, msobj);
		});
	};
}
